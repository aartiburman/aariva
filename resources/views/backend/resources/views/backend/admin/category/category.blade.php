...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row mb-4">
            ...
            @endphp
            <div class="col-md-3">
                <div class="card shadow-sm overflow-hidden {{ $bg_subtle_colors[$idx] }}">
                    <div class="card-body p-3">
                        <div class="d-flex align-items-center gap-3">
                            <div class="avatar-lg bg-white p-1">
                            ...
                            <div>
                                ...
                                <p class="mb-0 fs-12 text-muted">Category</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body p-3">
                <form action="{{ route('category.list') }}" method="POST" id="category-filter-form" class="no-loader">
                    ...
                    <div class="row align-items-end g-2">
                        ...
                        <div class="col-md-1">
                            <a href="{{ route('category.list') }}" class="btn btn-sm btn-outline-secondary w-100">Reset</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header border-bottom-0 d-flex justify-content-between align-items-center gap-1">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="{{ route('export.categories', request()->all()) }}" class="btn btn-sm btn-outline-secondary">
            ...
            <div class="card-footer border-top-0 py-3">
                <div class="d-flex align-items-center justify-content-between">
                    <p class="text-muted mb-0 fs-13">Showing {{ $cat_data->firstItem() }} to {{ $cat_data->lastItem() }} of {{ $cat_data->total() }} {{ strtolower(__('messages.category')) }}</p>
    ...
    $(document).ready(function() {
        ...
        $('#category-filter-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                data: formData,
                beforeSend: function() {
                    $('tbody').html('<tr><td colspan="6" class="text-center py-4"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></td></tr>');
                },
                success: function(response) {
                    $('tbody').html(response);
                    // Update footer count
                    let rowCount = $('<div>').html(response).find('tr').length;
                    $('.card-footer p').text('Showing all ' + rowCount + ' category');
                },
                error: function(xhr) {
        ...
        // Trigger submit on change of select

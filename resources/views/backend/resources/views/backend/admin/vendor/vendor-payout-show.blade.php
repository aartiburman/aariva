    ...
    <div class="container-fluid">
        <div class="row align-items-center mb-3">
            ...
            <div class="col-6 text-end d-flex justify-content-end gap-2">
                <a href="{{ route('vendor.payout', ['frequency' => $frequency ?? 'monthly']) }}" class="btn btn-sm btn-outline-secondary">
                    Back to {{ ucfirst($frequency ?? 'monthly') }} Payouts
                </a>
                <button type="button" id="btnGenerateHistory" class="btn btn-sm btn-primary" data-id="{{ $payout->id }}">Generate Wallet History</button>
            </div>
        </div>
        <div class="row g-3">
            <div class="col-lg-7">
            ...
            <div class="col-lg-5">
                ...
                <div class="card border-0 shadow-sm mt-3">
                    <div class="card-header bg-transparent border-0">
                        <h5 class="card-title mb-0 fw-bold">Vendor</h5>
                    </div>
                    <div class="card-body">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        btn.addEventListener('click', async function() {
            ...
            try {
                ...
                const resp = await fetch(`{{ url('vendor-payout') }}/${id}/wallet-history`, {
                    method: 'POST',
                    headers: {
                ...
                if (data.status) {
                    if (window.toastr) {
                        toastr.success(data.message || 'History generated successfully');
                    } else {
                        alert(data.message || 'Success');
                    }
                    // Optional: reload after a delay to see new transactions in the table

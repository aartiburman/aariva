                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4 pb-4 border-bottom border-light">
                            ...
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="mb-0 text-body">{{ $ticket->messages->first()->message ?? 'No content' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="conversation-wrapper" class="conversation-wrapper pe-2" style="max-height: 500px; overflow-y: auto;">
                            @foreach($ticket->messages as $index => $message)
                                ...
                                @else
                                <div class="d-flex mb-4 {{ $message->user_id === Auth::id() ? 'justify-content-end' : '' }}">
                                    ...
                                    <div style="max-width: 80%;">
                                        <div class="p-3 rounded-4 mb-2 shadow-sm {{ $message->user_id === Auth::id() ? 'bg-ticket-me text-white bubble-me' : 'bg-light text-dark bubble-other' }}">
                                            ...
                                            <div class="mt-2 pt-2 border-top {{ $message->user_id === Auth::id() ? 'border-white-10' : 'border-dark-10' }}">
                                                <a href="{{ asset('storage/' . $message->attachment) }}" target="_blank" 
                                                   class="d-inline-flex align-items-center fs-12 fw-medium {{ $message->user_id === Auth::id() ? 'text-white' : 'text-ticket-primary' }}">
                                                    <iconify-icon icon="solar:file-download-linear" class="me-1 fs-16"></iconify-icon>
                                                    {{ basename($message->attachment) }}
                                                </a>
                                            </div>
                                            @endif
                                        </div>
                                        <div class="d-flex {{ $message->user_id === Auth::id() ? 'justify-content-end' : '' }} align-items-center px-1">
                        ...
                        @if($ticket->status !== 'Closed')
                            ...
                            @else
                                <div class="reply-section mt-4 pt-4 border-top border-light">
                                    <form id="reply-form" action="{{ route('tickets.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="mb-3">
                                            <label class="form-label fw-bold fs-13 text-body">Quick Reply</label>
                                            <textarea name="message" class="form-control border-0 shadow-none bg-light p-3 rounded-4" rows="4" placeholder="Type your message here..." required></textarea>
                                        </div>
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="flex-grow-1 me-3">
                                                <div class="position-relative">
                                                    ...
                                                    <small class="text-muted ms-2 fs-11">Max 2MB</small>
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#ticketReplyAttachment').on('change', function(e) {
            const fileName = e.target.files[0]?.name || 'Add Attachment';
        ...
        function appendMessage(msg) {
            ...
            const attachmentHtml = msg.attachment ? `
                <div class="mt-2 pt-2 border-top ${isMe ? 'border-white-10' : 'border-dark-10'}">
                    <a href="/storage/${msg.attachment}" target="_blank" 
                       class="d-inline-flex align-items-center fs-12 fw-medium ${isMe ? 'text-white' : 'text-ticket-primary'}">
                        <iconify-icon icon="solar:file-download-linear" class="me-1 fs-16"></iconify-icon>
            ...
            const html = `
                <div class="d-flex mb-4 ${alignClass}">
                    ...
                    <div style="max-width: 80%;">
                        ...
                        <div class="d-flex ${isMe ? 'justify-content-end' : ''} align-items-center px-1">

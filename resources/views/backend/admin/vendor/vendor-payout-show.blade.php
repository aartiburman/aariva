    ...
    <div class="container-fluid">
        <div class="row align-items-center mb-3">
            ...
            <div class="col-6 text-end d-flex justify-content-end gap-2">
                <a href="{{ route('vendor.payout', ['frequency' => $frequency ?? 'daily']) }}" class="btn btn-sm btn-outline-secondary">
                    Back to {{ ucfirst($frequency ?? 'daily') }} Payouts
                </a>
            </div>
        </div>
        <div class="row g-3">
            <div class="col-lg-7">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-transparent border-0">
                        <h5 class="card-title mb-0 fw-bold">Payout Summary</h5>
                    </div>
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <div class="text-muted fs-12">Vendor</div>
                                <div class="fs-14 fw-medium">{{ $payout->vendor->store_name ?? $payout->vendor->name ?? 'N/A' }}</div>
                            </div>
                            <div class="col-md-6">
                                ...
                                @php
                                  ...
                                  elseif ($s === 'Unpaid') $cls = 'bg-warning-subtle text-warning';
                                @endphp
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <div class="fs-16 fw-bold">{{ optional(optional($payout->vendor)->country)->currency_code ?? 'AED' }} {{ number_format($payout->order_amount, 2) }}</div>
                            </div>
                            <div class="col-md-4">
                                <div class="text-muted fs-12">Commission</div>
                                <div class="fs-16">{{ optional(optional($payout->vendor)->country)->currency_code ?? 'AED' }} {{ number_format($payout->commission_amount, 2) }}</div>
                            </div>
                            <div class="col-md-4">
                                <div class="text-muted fs-12">Payout Amount</div>
                                <div class="fs-16 fw-bold">{{ optional(optional($payout->vendor)->country)->currency_code ?? 'AED' }} {{ number_format($payout->payout_amount, 2) }}</div>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <div class="text-muted fs-12">Transaction ID</div>
                                <div class="fs-14">{{ $payout->transaction_id ?: 'N/A' }}</div>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="text-muted fs-12">Note</div>
                                <div class="fs-14">{{ $payout->note ?: '—' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-5">
                    ...
                    <div class="card-body">
                        @if($payout->order)
                            <div class="mb-2 fs-14">
                                ...
                                <a href="{{ route('orders.details', $payout->order->order_reference_id ?? '') }}" class="fw-medium">#{{ $payout->order->order_reference_id ?? $payout->order_id }}</a>
                            </div>
                            <div class="mb-2 fs-14">
                ...
                <div class="card border-0 shadow-sm mt-3">
                    <div class="card-header bg-transparent border-0">
                        <h5 class="card-title mb-0 fw-bold">Vendor Wallet</h5>
                    </div>
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-3">
                            <div class="avatar-md bg-primary-subtle d-flex align-items-center justify-content-center">
                                <iconify-icon icon="solar:shop-2-linear" class="fs-24 text-primary"></iconify-icon>
                            </div>
                            <div>
                                <div class="fw-bold">{{ $payout->vendor->store_name ?? $payout->vendor->name ?? 'N/A' }}</div>
                                <div class="text-muted fs-12">{{ $payout->vendor->email ?? '' }}</div>
                            </div>
                        </div>
                        <hr class="my-3">
                        <div class="row g-2">
                            <div class="col-6">
                                <div class="text-muted fs-12">Current Wallet Balance</div>
                                <div class="fs-18 fw-bold text-primary">{{ number_format($payout->vendor->wallet_balance ?? 0, 2) }}</div>
                            </div>
                            <div class="col-6">
                                <div class="text-muted fs-12">Total Paid (Lifetime)</div>
                                <div class="fs-18 fw-bold text-success">{{ number_format(\App\Models\VendorPayout::where('vendor_id', $payout->vendor_id)->where('status', 'paid')->sum('payout_amount'), 2) }}</div>
                            </div>
                        </div>
                        <div class="mt-3 fs-14">
                            <div class="text-muted">Country</div>
                            <div>{{ $payout->vendor->country->name ?? 'N/A' }}</div>
                        </div>
                    </div>
                </div>
            </div>

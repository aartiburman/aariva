    ...
    <div class="container-xxl">
                ...
                <div class="card shadow-sm border-0 h-100 bg-primary text-white overflow-hidden">
                    <div class="card-body position-relative">
                        <div class="d-flex align-items-center gap-3">
                            <div class="avatar-md bg-opacity-25 rounded d-flex align-items-center justify-content-center">
        ...
        <div class="row g-4">
            ...
            <div class="col-xl-4">
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-body text-center">
                        <div class="position-relative d-inline-block mb-3">
                            ...
                            <span class="position-absolute bottom-0 end-0 p-1 rounded-circle shadow-sm">
                                @if($vendor->status == 1)
                    ...
                    <div class="card-footer bg-light-subtle border-top p-0">
                        <div class="list-group list-group-flush border-0">
                            <div class="list-group-item bg-transparent d-flex align-items-center gap-3 py-3">
                                <div class="avatar-sm shadow-sm rounded d-flex align-items-center justify-content-center">
                ...
                <div class="card shadow-sm border border-primary-subtle bg-light-subtle mb-4">
                    <div class="card-body">
                        ...
                        @if($vendor->device_token || $vendor->onesignal_player_id)
                            <div class="mb-3 p-2 rounded border border-dashed border-success-subtle">
                                <p class="text-success fs-11 mb-1 fw-medium"><iconify-icon icon="solar:check-circle-bold" class="me-1"></iconify-icon> Vendor is reachable</p>
                                <code class="fs-10 text-muted text-break">Token: {{ $vendor->device_token ?? $vendor->onesignal_player_id }}</code>
                            </div>
                        @else
            ...
            <div class="col-xl-8">
                ...
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-transparent p-0 border-bottom overflow-x-auto overflow-y-hidden">
                        <ul class="nav nav-tabs nav-tabs-custom nav-success border-bottom-0 flex-nowrap" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active py-3 px-2 text-nowrap" data-bs-toggle="tab" href="#banking-details" role="tab">
                                    <iconify-icon icon="solar:bank-linear" class="fs-18 me-1 align-middle"></iconify-icon>
                                    Banking & Tax
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link py-3 px-2 text-nowrap" data-bs-toggle="tab" href="#vendor-documents" role="tab">
                            ...
                            <li class="nav-item">
                                <a class="nav-link py-3 px-2 text-nowrap" data-bs-toggle="tab" href="#latest-products" role="tab">
                                    <iconify-icon icon="solar:box-linear" class="fs-18 me-1 align-middle"></iconify-icon>
                                    Recent Products
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link py-3 px-2 text-nowrap" data-bs-toggle="tab" href="#product-reviews" role="tab">
                                    <iconify-icon icon="solar:star-linear" class="fs-18 me-1 align-middle"></iconify-icon>
                                    Reviews & Ratings
                                    <span class="badge bg-warning rounded-pill ms-1">{{ $vendor->review_count }}</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div class="card-body p-4">
                        <div class="tab-content">
                                ...
                                <div class="row g-4">
                                    <div class="col-md-6">
                                        <div class="p-3 bg-light rounded border border-dashed border-primary border-opacity-25">
                                            ...
                                            <p class="mb-0 text-muted fs-13">Branch Location: {{ $vendor->branch_location ?: 'N/A' }}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div class="p-3 bg-light rounded border border-dashed border-info border-opacity-25 h-100">
                                            ...
                                            <div class="d-flex flex-column gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="fw-bold">{{ $vendor->vendor_tax ?: 'N/A' }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="tab-pane" id="product-reviews" role="tabpanel">
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center gap-2">
                                                        <div class="avatar-xs bg-soft-primary rounded-circle d-flex align-items-center justify-content-center">
                                                            <span class="fs-10 fw-bold text-primary">{{ strtoupper(substr($review->user->name ?? 'U', 0, 1)) }}</span>
                                                        </div>
                                                        <span class="fs-13 fw-medium text-dark">{{ $review->user->name ?? 'Unknown' }}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <a href="{{ route('product.detail', $review->product_id) }}" class="text-dark fs-13">{{ Str::limit($review->product->name ?? 'N/A', 25) }}</a>
                                                </td>
                                                <td>
                                                    <div class="text-warning d-flex gap-0">
                                                        @for($i = 1; $i <= 5; $i++)
                                                            <iconify-icon icon="solar:star-{{ $i <= $review->rating ? 'bold' : 'linear' }}" class="fs-14"></iconify-icon>
                                                        @endfor
                                                    </div>
                                                </td>
                                                <td>
                                                    <p class="mb-0 fs-12 text-muted" title="{{ $review->review }}">{{ Str::limit($review->review, 40) }}</p>
                                                </td>
                                                <td><span class="fs-12 text-muted">{{ $review->created_at->format('d M, Y') }}</span></td>
                                                <td>
                                                    @if($review->status == 1)
                                                    <span class="badge bg-success-subtle text-success fs-10">APPROVED</span>
                                                    @else
                                                    <span class="badge bg-warning-subtle text-warning fs-10">PENDING</span>
                                                    @endif
                                                </td>
                                            </tr>
                                            @empty
                                            <tr>
                                                <td colspan="6" class="text-center py-4 text-muted">No reviews found for this vendor's products.</td>
                                            </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                                ...
                                @if($reviews->hasPages())
                                <div class="mt-3">
                                    {{ $reviews->appends(['tab' => 'product-reviews'])->links() }}
                                </div>
                                @endif
                            </div>
                        </div>

...
class ErrorFactory {
    ...
    create(e, ...t) {
        const n = t[0] || {},
            a = `${this.service}/${e}`,
            o = this.errors[e],
            i = o ? function replaceTemplate(e, t) {
...
function openDB(e, t, {
    ...
    })), n && i.addEventListener("blocked", (e => n(e.oldVersion, e.newVersion, e))), s.then((e => {
        o && e.addEventListener("close", (() => o())), a && e.addEventListener("versionchange", (e => a(e.oldVersion, e.newVersion, e)))
    })).catch((() => {})), s
}
...
function deleteDB(e, {
    ...
    return t && n.addEventListener("blocked", (e => t(e.oldVersion, e))), wrap(n).then((() => {}))
}
...
function getMethod(e, t) {
    ...
    const n = t.replace(/FromIndex$/, ""),
        r = t !== n,
        a = g.includes(n);
    if (!(n in (r ? IDBIndex : IDBObjectStore).prototype) || !a && !l.includes(n)) return;
    const method = async function(e, ...t) {
        const o = this.transaction(e, a ? "readwrite" : "readonly");
        let i = o.store;
        return r && (i = i.index(t.shift())), (await Promise.all([i[n](...t), a && o.done]))[0]
    };
    ...
    callFidChangeCallbacks(n, t),
        function broadcastFidChange(e, t) {
            const n = function getBroadcastChannel() {
                !S && "BroadcastChannel" in self && (S = new BroadcastChannel("[Firebase] FID Change"), S.onmessage = e => {
...
let S = null;
const E = "firebase-installations-store";
let D = null;

function getDbPromise$1() {
    return D || (D = openDB("firebase-installations-database", 1, {
        upgrade: (e, t) => {
...
async function set(e, t) {
    const n = getKey$1(e),
        r = (await getDbPromise$1()).transaction(E, "readwrite"),
        a = r.objectStore(E),
        o = await a.get(n);
...
async function remove(e) {
    const t = getKey$1(e),
        n = (await getDbPromise$1()).transaction(E, "readwrite");
...
const C = "installations",
    ...
    internalFactory = e => {
        const t = e.getProvider("app").getImmediate(),
            n = _getProvider(t, C).getImmediate();
        return {
...
! function registerInstallations() {
    t(new Component(C, publicFactory, "PUBLIC")), t(new Component("installations-internal", internalFactory, "PRIVATE"))
}(), e(h, w), e(h, w, "esm2020");
const P = "BDOU99-h67HcA6JeFXHbSNMu7e2yNNu3RzoMj8TM4W88jITfq7ZmPvIM1Iv-4_l2LxQcYwhqby2xGpWwzjfAnG4",
    A = "google.c.a.c_id",
    M = 1e4;
var B, $;
...
const R = "fcm_token_details_db",
    F = "fcm_token_object_Store";
const K = "firebase-messaging-store";
let O = null;

function getDbPromise() {
    return O || (O = openDB("firebase-messaging-database", 1, {
        upgrade: (e, t) => {
            if (0 === t) e.createObjectStore(K)
        }
    })), O
}
async function dbGet(e) {
    const t = getKey(e),
        ...
        r = await n.transaction(K).objectStore(K).get(t);
    if (r) return r;
    {
        const t = await async function migrateOldDatabase(e) {
            if ("databases" in indexedDB) {
                const e = (await indexedDB.databases()).map((e => e.name));
                if (!e.includes(R)) return null
            }
            let t = null;
            return (await openDB(R, 5, {
                    upgrade: async (n, r, a, o) => {
                        if (r < 2) return;
                        if (!n.objectStoreNames.contains(F)) return;
                        const i = o.objectStore(F),
                            s = await i.index("fcmSenderId").get(e);
                        if (await i.clear(), s)
                            if (2 === r) {
                                const e = s;
                ...
                })).close(), await deleteDB(R), await deleteDB("fcm_vapid_details_db"), await deleteDB("undefined"),
...
async function dbSet(e, t) {
    const n = getKey(e),
        r = (await getDbPromise()).transaction(K, "readwrite");
    return await r.objectStore(K).put(t, n), await r.done, t
}
...
const j = new ErrorFactory("messaging", "Messaging", {
    "missing-app-config-values": 'Missing App configuration value: "{$valueName}"',
...
async function requestDeleteToken(e, t) {
    ...
    try {
        ...
        if (a.error) {
            ...
            throw j.create("token-unsubscribe-failed", {
                errorInfo: e
            })
        }
    } catch (e) {
...
function getBody({
    ...
    return r !== P && (a.web.applicationPubKey = r), a
}
    ...
    if (r) {
        if (function isTokenValid(e, t) {
                const n = t.vapidKey === e.vapidKey,
                    ...
                    o = t.p256dh === e.p256dh;
                return n && r && a && o
            }(r.subscriptionOptions, n)) return Date.now() >= r.createTime + 6048e5 ? async function updateToken(e, t) {
            try {
                const n = await async function requestUpdateToken(e, t) {
                    ...
                    try {
                        ...
                        throw j.create("token-update-failed", {
                            errorInfo: e?.toString()
                        })
                    }
                    if (o.error) {
                    ...
                    if (!o.token) throw j.create("token-update-no-token");
                    return o.token
                }(e.firebaseDependencies, t), r = {
                    ...t,
                    token: n,
                    createTime: Date.now()
                };
...
async function deleteTokenInternal(e) {
    ...
    t && (await requestDeleteToken(e.firebaseDependencies, t.token), await async function dbRemove(e) {
        const t = getKey(e),
            n = (await getDbPromise()).transaction(K, "readwrite");
        await n.objectStore(K).delete(t), await n.done
...
async function getNewToken(e, t) {
    const n = await async function requestGetToken(e, t) {
        ...
        try {
            ...
            throw j.create("token-subscribe-failed", {
                errorInfo: e?.toString()
            })
        }
        if (o.error) {
        ...
        if (!o.token) throw j.create("token-subscribe-no-token");
        return o.token
    }(e, t), r = {
        token: n,
        createTime: Date.now(),
...
class MessagingService {
    constructor(e, t, n) {
        ...
        const r = function extractAppConfig(e) {
            ...
            for (const e of t)
                if (!n[e]) throw getMissingValueError(e);
            return {
                appName: e.name,
...
async function registerDefaultSw(e) {
    try {
        e.swRegistration = await navigator.serviceWorker.register("url(/backend/js/firebase-messaging-sw.js)", {
            scope: "/firebase-cloud-messaging-push-scope"
        }), e.swRegistration.update().catch((() => {})), await async function waitForRegistrationActive(e) {
            return new Promise(((t, n) => {
                const r = setTimeout((() => n(new Error("Service worker not registered after 10000 ms"))), M),
                    a = e.installing || e.waiting;
                e.active ? (clearTimeout(r), t()) : a ? a.onstatechange = e => {
                    "activated" === e.target?.state && (a.onstatechange = null, clearTimeout(r), t())
                } : (clearTimeout(r), n(new Error("No incoming service worker found.")))
            }))
        }(e.swRegistration)
    } catch (e) {
        throw j.create("failed-service-worker-registration", {
            browserErrorMessage: e?.message
        })
    }
}
async function getToken$1(e, t) {
    if (!navigator) throw j.create("only-available-in-window");
    if ("default" === Notification.permission && await Notification.requestPermission(), "granted" !== Notification.permission) throw j.create("permission-blocked");
    return await async function updateVapidKey(e, t) {
        t ? e.vapidKey = t : e.vapidKey || (e.vapidKey = P)
    }(e, t?.vapidKey), await async function updateSwReg(e, t) {
        if (t || e.swRegistration || await registerDefaultSw(e), t || !e.swRegistration) {
            if (!(t instanceof ServiceWorkerRegistration)) throw j.create("invalid-sw-registration");
            e.swRegistration = t
        }
    }(e, t?.serviceWorkerRegistration), getTokenInternal(e)
}
...
async function messageEventListener(e, t) {
    ...
    e.onMessageHandler && n.messageType === $.PUSH_RECEIVED && ("function" == typeof e.onMessageHandler ? e.onMessageHandler(externalizePayload(n)) : e.onMessageHandler.next(externalizePayload(n)));
    const r = n.data;
    (function isConsoleMessage(e) {
        return "object" == typeof e && !!e && A in e
    })(r) && "1" === r["google.c.a.e"] && await logToScion(e, n.messageType, r)
}
const x = "@firebase/messaging",
    q = "0.12.24",
...
async function isWindowSupported() {
    ...
    return "undefined" != typeof window && function isIndexedDBAvailable() {
        try {
    ...
    }() && "serviceWorker" in navigator && "PushManager" in window && "Notification" in window && "fetch" in window && ServiceWorkerRegistration.prototype.hasOwnProperty("showNotification") && PushSubscription.prototype.hasOwnProperty("getKey")
}

function getMessagingInWindow(e = n()) {
    return isWindowSupported().then((e => {
        if (!e) throw j.create("unsupported-browser")
    }), (e => {
        throw j.create("indexed-db-unsupported")
...
function onMessage(e, t) {
    ...
    t(new Component("messaging", WindowMessagingFactory, "PUBLIC")), t(new Component("messaging-internal", WindowMessagingInternalFactory, "PRIVATE")), e(x, q), e(x, q, "esm2020")
}();
export {
    deleteToken,
